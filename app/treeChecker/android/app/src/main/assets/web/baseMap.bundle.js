...
"use strict";
var _reactNativeWebviewMessaging=__webpack_require__(1);var _reactNativeWebviewMessaging2=_interopRequireDefault(_reactNativeWebviewMessaging);var _layerDefinitions=__webpack_require__(3);var _layerDefinitions2=_interopRequireDefault(_layerDefinitions);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}_reactNativeWebviewMessaging2.default.on('json',function(json){receiveWebViewMessages(json);});function sendDataToReact(data){console.log("Send data to React: "+JSON.stringify(data));_reactNativeWebviewMessaging2.default.sendJSON(data);};var mymap=L.map('mapid');var controlLayers=null;var geojsonLayer=null;var overlays={};mymap.on('load',function(e){addMapEvents();addMapControls();initLayers();});setTimeout(function(){sendDataToReact({action:"webInit"});},3000);function receiveWebViewMessages(json){if(json.bbox){var corner1=L.latLng(json.bbox[0],json.bbox[1]),corner2=L.latLng(json.bbox[2],json.bbox[3]),bounds=L.latLngBounds(corner1,corner2);mymap.fitBounds(bounds);}else if(json.latitude){mymap.setView([json.latitude,json.longitude],17);}if(json.obs){addMarkers(json.obs);}if(json.url){addOfflineLayers(json.url);}};function initLayers(){var osm=new L.TileLayer(_layerDefinitions2.default.osm.url,{maxZoom:19,attribution:_layerDefinitions2.default.osm.attribution});mymap.addLayer(osm);controlLayers.addBaseLayer(osm,_layerDefinitions2.default.osm.layerName);}function addOfflineLayers(baseURL){var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=_layerDefinitions2.default.downloadables[typeof Symbol==='function'?Symbol.iterator:'@@iterator'](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var layerName=_step.value;console.log("Adding "+layerName);var layer=new L.TileLayer(baseURL+'/tiles/'+layerName+'/{z}/{x}/{y}.png',{maxZoom:17});mymap.addLayer(layer);if(controlLayers)controlLayers.addOverlay(layer,layerName);else overlays[layerName]=layer;}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}}function addMapEvents(){mymap.on('contextmenu',function(e){sendDataToReact({action:'addObservation',latitude:e.latlng.lat,longitude:e.latlng.lng});});}function addMapControls(){controlLayers=new L.control.layers({},overlays,{sortLayers:true,hideSingleBase:true});controlLayers.addTo(mymap);L.control.locate().addTo(mymap);var comp=new L.Control.Compass({autoActive:true});mymap.addControl(comp);}function viewElement(id){sendDataToReact({action:'info',id:id});}function addMarkers(data){var geojson={"type":"FeatureCollection","features":[]};for(var index in data){var point=data[index];var geojsonPoint={"type":"Feature","geometry":{"type":"Point","coordinates":[point.position.longitude,point.position.latitude]},"properties":{"popup":buildSurveyDataPopup(point),"id":point.key}};geojson.features.push(geojsonPoint);}if(geojsonLayer!=null){controlLayers.removeLayer(geojsonLayer);mymap.removeLayer(geojsonLayer);}geojsonLayer=L.geoJSON(geojson,{pointToLayer:function pointToLayer(feature,latlng){return L.marker(latlng);},onEachFeature:function onEachFeature(feature,layer){layer.bindPopup(feature.properties.popup);}});geojsonLayer.addTo(mymap);controlLayers.addOverlay(geojsonLayer,'Own data');}function buildSurveyDataPopup(data){var container=L.DomUtil.create('div');container.innerHTML='<h2>'+data.name+'</h2><p><b>Specie:</b>'+data.tree_specie.name+'</p><p><b>Diameter:</b>'+data.crown_diameter.name+'</p>';var btn=L.DomUtil.create('i','fa fa-info-circle fa-2x centered',container);L.DomEvent.on(btn,'click',function(){viewElement(data.key);});return container;}
...
"use strict";
var LayerDefinitions={osm:{layerName:'OSM',url:'https://cartodb-basemaps-{s}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png',attribution:'Map data © <a href="http://openstreetmap.org">OpenStreetMap</a> contributors'},testWMS:{url:'http://ows.mundialis.de/services/service?',layers:'TOPO-OSM-WMS',format:'image/png',transparent:'true',version:'1.1.0',height:512,width:512,crs:'EPSG:4326'},testWMS2:{url:'http://ows.mundialis.de/services/service?',layers:'OSM-WMS',format:'image/png',transparent:'true',version:'1.1.0',height:512,width:512,crs:'EPSG:4326'},jrcOrtophotosWMS:{url:'https://ies-ows.jrc.es.europa.eu/canhemonVI?',layers:'pt.ortophotos.20151127',format:'image/png',transparent:'true',version:'1.1.0',height:512,width:512,crs:'EPSG:4326'},jrcGeometriesWMS:{url:'https://ies-ows.jrc.es.europa.eu/canhemonVI?',layers:'pt.centroid_polygons.20151127',format:'image/png',transparent:'true',version:'1.1.0',height:512,width:512,crs:'EPSG:4326'},downloadables:['testWMS','testWMS2']};module.exports=LayerDefinitions;
